...
namespace FMAplication.Services.WorkFlows.Implementation
{
    public class WorkFlowService : IWorkFlowService
    {
        ...
        public async Task<WorkFlowModel> SaveAsync(WorkFlowModel model)
        {
            ...
            var configs = _workflowConfig.Where(x => x.WorkFlowId == result.Id).ToList();
            var list = configs.Where(x => x.Sequence > result.WorkflowStep).ToList();
            await _workflowConfig.DeleteListAsync(list);
