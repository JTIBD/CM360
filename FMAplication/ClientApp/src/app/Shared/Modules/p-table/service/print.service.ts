...
export class PrintService {
    ...
    printContent(pTableSetting: IPTableSetting, pTableData: any[],pTableSummery:object ) {
        this.pTableData = pTableData
        let tableHeader = "", tableBody = "";
        let slNo = 0;
        //add SL 
        if (pTableSetting.enabledSerialNo) {
            tableHeader = tableHeader + ` <td  align="center">SL </td> `;
        }
        ...
        if(pTableSummery){
            printSummery = `<div>`        
                printSummery = `                    
                    ${printSummery}
                    <div>
                        ${Object.keys(pTableSummery).map(k=>{
                            return `
                                <p><span style="font-weight: bold;">${k}</span>:<span>${pTableSummery[k]}</span> </p>
                            `
                        }).join('\n')
                    }
                    </div>
